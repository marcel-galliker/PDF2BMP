...
//	}
	...
	if (m_ExitDlg->m_Permanent == TRUE) {
		if (m_ExitDlg->m_SelectedOpt == TRUE) {	//Minimize option
			...
			//CDialog::OnClose();
			//CPDFToBmpDlg::OnDestroy(); // vtt 2/1/2019
		}
	} else {
...
void CPDFToBmpDlg::OnEnChangeEnThreadCnt()
{
	...
	m_options.threadCnt = m_threadCnt;
}
...
int CPDFToBmpDlg::GetMACaddress()
{
	...
	line=0;
	mac[0]=0;
	system( "GetMAC > PDF2BMPmac.txt");
	err = fopen_s( &fili, "PDF2BMPmac.txt", "rb");
	if ( !err){
		...
		MessageBox( L"Please make a D partition", L"No D partition");
	}
...
//			if ( !strncmp( mac, &dStr[ 100], 17)){	// check mac
			if ( !strncmp( mac, &dStr[ 100], 12) || !strncmp( mac2, &dStr[ 100], 12)){	// check mac
				...
				MessageBox( L"Wrong license (mac). contact www.graphtech.us", L"Bad license");
				return true;
			}
		}
		else{
			MessageBox( L"Wrong license (PC name). contact www.graphtech.us", L"Bad license");
			return false;
		}
	}
	else{
		MessageBox( L"No license file. contact www.graphtech.us", L"Bad license");
	}

...
pdf_parse_link_dest(pdf_document *xref, pdf_obj *dest)
{
	...
	if (!strcmp("XYZ", pdf_to_name(obj)))
	{
		...
		ld.ld.gotor.flags |= base_link_flag_r_is_zoom;
	}
	else if ((!strcmp("Fit", pdf_to_name(obj))) || (!strcmp("FitB", pdf_to_name(obj))))
	{
		ld.ld.gotor.flags |= base_link_flag_fit_h;
		ld.ld.gotor.flags |= base_link_flag_fit_v;
	}
	...
	if (l_from_2)
	{
		...
		if (pdf_is_int(obj))
		{
			ld.ld.gotor.flags |= base_link_flag_l_valid;
	...
	if (b_from_3)
	{
		...
		if (pdf_is_int(obj))
		{
			ld.ld.gotor.flags |= base_link_flag_b_valid;
	...
	if (r_from_4)
	{
		...
		if (pdf_is_int(obj))
		{
			ld.ld.gotor.flags |= base_link_flag_r_valid;
	...
	if (t_from_5 || t_from_3 || t_from_2)
	{
		...
		if (pdf_is_int(obj))
		{
			ld.ld.gotor.flags |= base_link_flag_t_valid;
	...
	if ((ld.ld.gotor.flags & (base_link_flag_l_valid | base_link_flag_r_valid)) == base_link_flag_l_valid)
		ld.ld.gotor.rb.x = ld.ld.gotor.lt.x;
	if ((ld.ld.gotor.flags & (base_link_flag_l_valid | base_link_flag_r_valid | base_link_flag_r_is_zoom)) == base_link_flag_r_valid)
		ld.ld.gotor.lt.x = ld.ld.gotor.rb.x;
	if ((ld.ld.gotor.flags & (base_link_flag_t_valid | base_link_flag_b_valid)) == base_link_flag_t_valid)
		ld.ld.gotor.rb.y = ld.ld.gotor.lt.y;
	if ((ld.ld.gotor.flags & (base_link_flag_t_valid | base_link_flag_b_valid)) == base_link_flag_b_valid)
...
static base_link *
pdf_load_link(pdf_document *xref, pdf_obj *dict, base_matrix page_ctm)
{
	pdf_obj *dest = NULL;
	pdf_obj *action;
	pdf_obj *obj;
	...
	if (obj)
		...
		bbox = base_empty_rect;

	bbox = base_transform_rect(page_ctm, bbox);
	...
	if (ld.kind == base_LINK_NONE)
		return NULL;
	return base_new_link(ctx, bbox, ld);
}

base_link *
pdf_load_link_annots(pdf_document *xref, pdf_obj *annots, base_matrix page_ctm)
{
	base_link *link, *head, *tail;
	pdf_obj *obj;
	int i, n;
...
void
pdf_free_annot(base_context *ctx, pdf_annot *annot)
{
	pdf_annot *next;
...
{
	base_matrix matrix = annot->ap->matrix;
	base_rect bbox = annot->ap->bbox;
	base_rect rect = annot->rect;
	float w, h, x, y;
	...
	annot->matrix = base_concat(base_scale(w, h), base_translate(x, y));
}
	...
	for (i = 0; i < len; i++)
	{
		...
		if (pdf_is_dict(ap))
		{
			...
			if (pdf_is_stream(xref, pdf_to_num(n), pdf_to_gen(n)))
			{
				...
				base_catch(ctx)
				{
					base_warn(ctx, "ignoring broken annotation");
					continue;
				}

				annot = base_malloc_struct(ctx, pdf_annot);

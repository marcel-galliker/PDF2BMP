...
{
	return base_malloc_array(opaque, items, size);
}

static void zfree(void *opaque, void *address)
{
	base_free(opaque, address);
}
...
png_deinterlace(struct info *info, int *passw, int *passh, int *passofs)
{
	...
	output = base_malloc_array(info->ctx, info->height, stride);
	...
	base_free(info->ctx, info->samples);
	info->samples = output;
}
...
png_read_ihdr(struct info *info, unsigned char *p, int size)
{
	...
	if (size != 13)
		base_throw(info->ctx, "IHDR chunk is the wrong size");
	...
	if (info->width <= 0)
		base_throw(info->ctx, "image width must be > 0");
	if (info->height <= 0)
		base_throw(info->ctx, "image height must be > 0");

	if (info->depth != 1 && info->depth != 2 && info->depth != 4 &&
			info->depth != 8 && info->depth != 16)
		base_throw(info->ctx, "image bit depth must be one of 1, 2, 4, 8, 16");
	if (color == 2 && info->depth < 8)
		base_throw(info->ctx, "illegal bit depth for truecolor");
	if (color == 3 && info->depth > 8)
		base_throw(info->ctx, "illegal bit depth for indexed");
	if (color == 4 && info->depth < 8)
		base_throw(info->ctx, "illegal bit depth for grayscale with alpha");
	if (color == 6 && info->depth < 8)
		base_throw(info->ctx, "illegal bit depth for truecolor with alpha");
	...
	else if (color == 3) 
	{
		...
		base_throw(info->ctx, "unknown color type");

	if (compression != 0)
		base_throw(info->ctx, "unknown compression method");
	if (filter != 0)
		base_throw(info->ctx, "unknown filter method");
	if (info->interlace != 0 && info->interlace != 1)
		base_throw(info->ctx, "interlace method not supported");
}
...
png_read_plte(struct info *info, unsigned char *p, int size)
{
	...
	if (n > 256)
	{
		base_warn(info->ctx, "too many samples in palette");
		n = 256;
	}
...
png_read_trns(struct info *info, unsigned char *p, int size)
{
	...
	if (info->indexed)
	{
		if (size > 256)
		{
			base_warn(info->ctx, "too many samples in transparency table");
			size = 256;
		}
		for (i = 0; i < size; i++)
	...
	{
		if (size != info->n * 2)
			base_throw(info->ctx, "tRNS chunk is the wrong size");
		for (i = 0; i < info->n; i++)
...
png_read_idat(struct info *info, unsigned char *p, int size, z_stream *stm)
{
	...
	if (code != Z_OK && code != Z_STREAM_END)
		base_throw(info->ctx, "zlib error: %s", stm->msg);
	if (stm->avail_in != 0)
	{
		if (stm->avail_out == 0)
			base_throw(info->ctx, "ran out of output before input");
		base_throw(info->ctx, "inflate did not consume buffer (%d remaining)", stm->avail_in);
	}
}
...
{
	if (size != 9)
		base_throw(info->ctx, "pHYs chunk is the wrong size");
	if (p[8] == 1)
	{
...
static void
png_read_image(base_context *ctx, struct info *info, unsigned char *p, int total)
{
	...
	if (total < 8 + 12 || memcmp(p, png_signature, 8))
		base_throw(info->ctx, "not a png image (wrong signature)");
	...
	if (size + 12 > total)
		base_throw(info->ctx, "premature end of data in png image");

	if (!memcmp(p + 4, "IHDR", 4))
		png_read_ihdr(info, p + 8, size);
	else
		base_throw(info->ctx, "png file must start with IHDR chunk");
	...
	info->samples = base_malloc(ctx, info->size);
	...
	if (code != Z_OK)
	{
		base_free(ctx, info->samples);
		base_throw(ctx, "zlib error: %s", stm.msg);
	}
...
static base_pixmap *
png_expand_palette(base_context *ctx, struct info *info, base_pixmap *src)
{
	base_pixmap *dst = base_new_pixmap(ctx, base_device_rgb, src->w, src->h);
	...
	base_drop_pixmap(info->ctx, src);
	return dst;
}

static void
png_mask_transparency(struct info *info, base_pixmap *dst)
{
...
base_pixmap *
base_load_png(base_context *ctx, unsigned char *p, int total)
{
	base_pixmap *image;
	...
	base_try(ctx)
	{
		image = base_new_pixmap(ctx, colorspace, png.width, png.height);
	}
	base_catch(ctx)
	{
		base_free(png.ctx, png.samples);
	...
	base_unpack_tile(image, png.samples, png.n, png.depth, stride, png.indexed);
	...
	if (png.transparency || png.n == 2 || png.n == 4)
		base_premultiply_pixmap(png.ctx, image);

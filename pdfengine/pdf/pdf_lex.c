...
static int
lex_number(base_stream *f, pdf_lexbuf *buf, int c)
{
	int neg = 0;
	int i = 0;
	int n;
...
static void
lex_name(base_stream *f, pdf_lexbuf *buf)
{
	char *s = buf->scratch;
	int n = buf->size;
...
static int
lex_string(base_stream *f, char *buf, int n)
{
	char *s = buf;
	char *e = buf + n;
	int bal = 1;
...
static int
lex_hex_string(base_stream *f, char *buf, int n)
{
	char *s = buf;
	char *e = buf + n;
	int a = 0, x = 0;
	...
	while (s < e)
	{
		...
		case EOF:
			goto end;
		default:
			base_warn(f->ctx, "ignoring invalid character in hex string: '%c'", c);
		}
	}
end:
	return s - buf;
...
int
pdf_lex(base_stream *f, pdf_lexbuf *buf)
{
	while (1)
	{
		int c = base_read_byte(f);
		...
		case ')':
			base_warn(f->ctx, "lexical error (unexpected ')')");
			continue;
		case '<':
			c = base_read_byte(f);
			if (c == '<')

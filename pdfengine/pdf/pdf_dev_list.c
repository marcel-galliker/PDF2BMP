...
static void
base_list_fill_path(base_device *dev, base_path *path, int even_odd, base_matrix ctm,
	base_colorspace *colorspace, float *color, float alpha)
{
	base_display_node *node;
...
static void
base_list_stroke_path(base_device *dev, base_path *path, base_stroke_state *stroke, base_matrix ctm,
	base_colorspace *colorspace, float *color, float alpha)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_clip_path(base_device *dev, base_path *path, base_rect *rect, int even_odd, base_matrix ctm)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_clip_stroke_path(base_device *dev, base_path *path, base_rect *rect, base_stroke_state *stroke, base_matrix ctm)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_fill_text(base_device *dev, base_text *text, base_matrix ctm,
	base_colorspace *colorspace, float *color, float alpha)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_stroke_text(base_device *dev, base_text *text, base_stroke_state *stroke, base_matrix ctm,
	base_colorspace *colorspace, float *color, float alpha)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_clip_text(base_device *dev, base_text *text, base_matrix ctm, int accumulate)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_clip_stroke_text(base_device *dev, base_text *text, base_stroke_state *stroke, base_matrix ctm)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_ignore_text(base_device *dev, base_text *text, base_matrix ctm)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_pop_clip(base_device *dev)
{
	base_display_node *node;
...
static void
base_list_fill_shade(base_device *dev, base_shade *shade, base_matrix ctm, float alpha)
{
	base_display_node *node;
	base_context *ctx = dev->ctx;
...
static void
base_list_fill_image(base_device *dev, base_image *image, base_matrix ctm, float alpha)
{
	base_display_node *node;
...
static void
base_list_fill_image_mask(base_device *dev, base_image *image, base_matrix ctm,
	base_colorspace *colorspace, float *color, float alpha)
{
	base_display_node *node;
...
static void
base_list_clip_image_mask(base_device *dev, base_image *image, base_rect *rect, base_matrix ctm)
{
	base_display_node *node;
...
static void
base_list_begin_mask(base_device *dev, base_rect rect, int luminosity, base_colorspace *colorspace, float *color)
{
	base_display_node *node;
...
static void
base_list_end_mask(base_device *dev)
{
	base_display_node *node;
...
static void
base_list_begin_group(base_device *dev, base_rect rect, int isolated, int knockout, int blendmode, float alpha)
{
	base_display_node *node;
...
static void
base_list_end_group(base_device *dev)
{
	base_display_node *node;
...
static void
base_list_begin_tile(base_device *dev, base_rect area, base_rect view, float xstep, float ystep, base_matrix ctm)
{
	base_display_node *node;
...
static void
base_list_end_tile(base_device *dev)
{
	base_display_node *node;
...
void base_watermark_process(base_display_list *list, base_device *dev, base_bbox org_bbox, base_bbox scissor, wchar_t* strWatermark, double dAngle, double dAlpha)
{
	base_display_node *node;
	base_text *text;
	wchar_t c;
	...
	i = 0;
	while ( c = strWatermark[i] )
	{
		// bangak
		if ( c >= 0x0020 && c <= 0x007E )
			text->items[i].gid = c - 29;
		if ( c == 0x007F )
			text->items[i].gid = 2;
		...
		if ( c >= 0x4E00 && c <= 0x9FA5 )
			text->items[i].gid = c - 18544;

		// japanese
		if ( c >= 0x3041 && c <= 0x3093 )
			text->items[i].gid = c - 11424;
		if ( c >= 0x30A1 && c <= 0x30F6 )
			text->items[i].gid = c - 11430;

		// ---------------Batang-------------------
		// korean
		if ( c >= 0xAC00 && c <= 0xD7A3 )
			text->items[i].gid = c - 21706;
	...
	my_base_fill_text(dev, node->item.text, node->ctm, node->colorspace, node->color, dAlpha, org_bbox, scissor, dAngle);
}

/***************************************************************************************/
/* function name:	base_new_list_device
/* description:		create base_device variable
/* param 1:			pointer to the context
/* param 2:			pointer to the base_display_list
...
base_device *
base_new_list_device(base_context *ctx, base_display_list *list)
{
	base_device *dev = base_new_device(ctx, list);

	dev->fill_path = base_list_fill_path;
	dev->stroke_path = base_list_stroke_path;
	dev->clip_path = base_list_clip_path;
	dev->clip_stroke_path = base_list_clip_stroke_path;

	dev->fill_text = base_list_fill_text;
	dev->stroke_text = base_list_stroke_text;
	dev->clip_text = base_list_clip_text;
	dev->clip_stroke_text = base_list_clip_stroke_text;
	dev->ignore_text = base_list_ignore_text;

	dev->fill_shade = base_list_fill_shade;
	dev->fill_image = base_list_fill_image;
	dev->fill_image_mask = base_list_fill_image_mask;
	dev->clip_image_mask = base_list_clip_image_mask;

	dev->pop_clip = base_list_pop_clip;

	dev->begin_mask = base_list_begin_mask;
	dev->end_mask = base_list_end_mask;
	dev->begin_group = base_list_begin_group;
	dev->end_group = base_list_end_group;

	dev->begin_tile = base_list_begin_tile;
	dev->end_tile = base_list_end_tile;

	return dev;
}

/***************************************************************************************/
/* function name:	base_new_display_list
/* description:		create base_display_list variable
/* param 1:			pointer to the context
/* return:			newly created base_display_list variable
/* creator & modifier : 
/* create:			2012.09.10 ~
/* working:			2012.09.10 ~
/***************************************************************************************/
...
base_display_list *
base_new_display_list(base_context *ctx)
{
...
/***************************************************************************************/
/* function name:	base_free_display_list
/* description:		free base_display_list variable
/* param 1:			pointer to the context
/* param 2:			base_display_list to free
/* return:			none
/* creator & modifier : 
/* create:			2012.09.10 ~
/* working:			2012.09.10 ~
...
void
base_free_display_list(base_context *ctx, base_display_list *list)
{
	base_display_node *node;
...
/***************************************************************************************/
/* function name:	base_run_display_list
/* description:		reads the contents of the page into base_display_list
/* param 1:			base_display_list
/* param 2:			base_device
/* param 3:			ctm matrix
/* param 4:			scissor
/* param 5:			cookie
/* param 6:			whether only the text or not
/* return:			none
/* creator & modifier : 
/* create:			2012.09.10 ~
/* working:			2012.09.10 ~
...
void
base_run_display_list(base_display_list *list, base_device *dev, base_matrix top_ctm, base_bbox scissor, base_cookie *cookie, char bOnlyText)
{
	base_display_node *node;
	base_matrix ctm;
	base_rect rect;

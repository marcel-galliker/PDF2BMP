...
pdf_cmap_table g_customCMapTable[1024] = { 0 };
int g_customCMapTableSize = 0;
...
pdf_cmap *
pdf_load_builtin_cmap(base_context *ctx, char *cmap_name)
{
	int i;
	for (i = 0; i < g_customCMapTableSize; i++)
	{
		if (strcmp(cmap_name, g_customCMapTable[i].name) == 0)
			return g_customCMapTable[i].cmap;
	}

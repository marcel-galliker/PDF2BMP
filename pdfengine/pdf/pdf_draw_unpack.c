...
void
base_unpack_tile(base_pixmap *dst, unsigned char * restrict src, int n, int depth, int stride, int scale)
{
	int pad, x, y, k;
	int w = dst->w;
...
void
base_decode_indexed_tile(base_pixmap *pix, float *decode, int maxval)
{
	int add[base_MAX_COLORS];
	int mul[base_MAX_COLORS];
...
void
base_decode_tile(base_pixmap *pix, float *decode)
{
	int add[base_MAX_COLORS];
	int mul[base_MAX_COLORS];
	unsigned char *p = pix->samples;
	...
	while (len--)
	{
		for (k = 0; k < n; k++)
		{
			int value = add[k] + base_mul255(p[k], mul[k]);
			p[k] = CLAMP(value, 0, 255);
		}
		p += pix->n;
	}

...
static void paint_scan(base_pixmap *pix, int y, int x1, int x2, int *v1, int *v2, int n)
{
	unsigned char *p = pix->samples + ((y - pix->y) * pix->w + (x1 - pix->x)) * pix->n;
	int v[base_MAX_COLORS];
	int dv[base_MAX_COLORS];
	int w = x2 - x1;
	int k;
...
static void
base_paint_triangle(base_pixmap *pix, float *av, float *bv, float *cv, int n, base_bbox bbox)
{
	...
	for (i = 0; i < len; i++)
	{
		gel[i][0] = floorf(poly[i][0] + 0.5f) * 65536; 
		gel[i][1] = floorf(poly[i][1] + 0.5f);	
		for (k = 2; k < n; k++)
			gel[i][k] = poly[i][k] * 65536;	
	}
...
static void
base_paint_quad(base_pixmap *pix,
		base_point p0, base_point p1, base_point p2, base_point p3,
		float c0, float c1, float c2, float c3,
		int n, base_bbox bbox)
{
	float v[4][3];
	...
	base_paint_triangle(pix, v[0], v[2], v[3], n, bbox);
	base_paint_triangle(pix, v[0], v[3], v[1], n, bbox);
}
...
static base_point
base_point_on_circle(base_point p, float r, float theta)
{
...
static void
base_paint_linear(base_shade *shade, base_matrix ctm, base_pixmap *dest, base_bbox bbox)
{
	base_point p0, p1;
	base_point v0, v1, v2, v3;
	base_point e0, e1;
	float theta;

	p0.x = shade->mesh[0];
	p0.y = shade->mesh[1];
	p0 = base_transform_point(ctm, p0);

	p1.x = shade->mesh[3];
	p1.y = shade->mesh[4];
	p1 = base_transform_point(ctm, p1);

	theta = my_atan2f(p1.y - p0.y, p1.x - p0.x);
	theta += (float)M_PI * 0.5f;

	v0 = base_point_on_circle(p0, HUGENUM, theta);
	v1 = base_point_on_circle(p1, HUGENUM, theta);
	v2 = base_point_on_circle(p0, -HUGENUM, theta);
	v3 = base_point_on_circle(p1, -HUGENUM, theta);

	base_paint_quad(dest, v0, v1, v2, v3, 0, 255, 0, 255, 3, bbox);
	...
	if (shade->extend[0])
	{
		...
		base_paint_quad(dest, e0, e1, v0, v2, 0, 0, 0, 0, 3, bbox);
	}
	...
	if (shade->extend[1])
	{
		...
		base_paint_quad(dest, e0, e1, v1, v3, 255, 255, 255, 255, 3, bbox);
	}
}

static void
base_paint_annulus(base_matrix ctm,
		base_point p0, float r0, float c0,
		base_point p1, float r1, float c1,
		base_pixmap *dest, base_bbox bbox)
{
	base_point t0, t1, t2, t3, b0, b1, b2, b3;
	float theta, step;
	int i;
	...
	{
		t0 = base_point_on_circle(p0, r0, theta + i * step);
		t1 = base_point_on_circle(p0, r0, theta + i * step + step);
		t2 = base_point_on_circle(p1, r1, theta + i * step);
		t3 = base_point_on_circle(p1, r1, theta + i * step + step);
		b0 = base_point_on_circle(p0, r0, theta - i * step);
		b1 = base_point_on_circle(p0, r0, theta - i * step - step);
		b2 = base_point_on_circle(p1, r1, theta - i * step);
		b3 = base_point_on_circle(p1, r1, theta - i * step - step);

		t0 = base_transform_point(ctm, t0);
		t1 = base_transform_point(ctm, t1);
		t2 = base_transform_point(ctm, t2);
		t3 = base_transform_point(ctm, t3);
		b0 = base_transform_point(ctm, b0);
		b1 = base_transform_point(ctm, b1);
		b2 = base_transform_point(ctm, b2);
		b3 = base_transform_point(ctm, b3);

		base_paint_quad(dest, t0, t1, t2, t3, c0, c0, c1, c1, 3, bbox);
		base_paint_quad(dest, b0, b1, b2, b3, c0, c0, c1, c1, 3, bbox);
	}
}

static void
base_paint_radial(base_shade *shade, base_matrix ctm, base_pixmap *dest, base_bbox bbox)
{
	base_point p0, p1;
	float r0, r1;
	base_point e;
	...
	if (shade->extend[0])
	{
		...
		base_paint_annulus(ctm, e, er, 0, p0, r0, 0, dest, bbox);
	}

	base_paint_annulus(ctm, p0, r0, 0, p1, r1, 255, dest, bbox);
	...
	if (shade->extend[1])
	{
		...
		base_paint_annulus(ctm, p1, r1, 255, e, er, 255, dest, bbox);
	}
}

static void
base_paint_mesh(base_context *ctx, base_shade *shade, base_matrix ctm, base_pixmap *dest, base_bbox bbox)
{
	float tri[3][MAXN];
	base_point p;
	float *mesh;
	...
	while (ntris--)
	{
		for (k = 0; k < 3; k++)
		{
			...
			{
				base_convert_color(ctx, dest->colorspace, tri[k] + 2, shade->colorspace, mesh);
				for (i = 0; i < dest->colorspace->n; i++)
					tri[k][i + 2] *= 255;
				mesh += shade->colorspace->n;
			}
		}
		base_paint_triangle(dest, tri[0], tri[1], tri[2], 2 + dest->colorspace->n, bbox);
	}
}

void
base_paint_shade(base_context *ctx, base_shade *shade, base_matrix ctm, base_pixmap *dest, base_bbox bbox)
{
	unsigned char clut[256][base_MAX_COLORS];
	base_pixmap *temp = NULL;
	base_pixmap *conv = NULL;
	...
	base_try(ctx)
	{
		...
		if (shade->use_function)
		{
			for (i = 0; i < 256; i++)
			{
				base_convert_color(ctx, dest->colorspace, color, shade->colorspace, shade->function[i]);
				for (k = 0; k < dest->colorspace->n; k++)
					clut[i][k] = color[k] * 255;
				clut[i][k] = shade->function[i][shade->colorspace->n] * 255;
			}
			conv = base_new_pixmap_with_bbox(ctx, dest->colorspace, bbox);
			temp = base_new_pixmap_with_bbox(ctx, base_device_gray, bbox);
			base_clear_pixmap(ctx, temp);
		}
		else
		{
			temp = dest;
		...
		switch (shade->type)
		{
		case base_LINEAR: base_paint_linear(shade, ctm, temp, bbox); break;
		case base_RADIAL: base_paint_radial(shade, ctm, temp, bbox); break;
		case base_MESH: base_paint_mesh(ctx, shade, ctm, temp, bbox); break;
		}
		...
		if (shade->use_function)
		{
			...
			{
				int v = *s++;
				int a = base_mul255(*s++, clut[v][conv->n - 1]);
				for (k = 0; k < conv->n - 1; k++)
					*d++ = base_mul255(clut[v][k], a);
				*d++ = a;
			}
			base_paint_pixmap(dest, conv, 255);
			base_drop_pixmap(ctx, conv);
			base_drop_pixmap(ctx, temp);
		}
	}
	base_catch(ctx)
	{

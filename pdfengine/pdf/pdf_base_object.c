...
pdf_obj *
pdf_new_null(base_context *ctx)
{
	pdf_obj *obj;
	obj = base_malloc(ctx, sizeof(pdf_obj));
	obj->ctx = ctx;
	obj->refs = 1;
	obj->kind = PDF_NULL;
	return obj;
...
pdf_obj *
pdf_new_bool(base_context *ctx, int b)
{
	pdf_obj *obj;
	obj = base_malloc(ctx, sizeof(pdf_obj));
	obj->ctx = ctx;
...
pdf_obj *
pdf_new_int(base_context *ctx, int i)
{
	pdf_obj *obj;
	obj = base_malloc(ctx, sizeof(pdf_obj));
	obj->ctx = ctx;
...
pdf_obj *
pdf_new_real(base_context *ctx, float f)
{
	pdf_obj *obj;
	obj = base_malloc(ctx, sizeof(pdf_obj));
	obj->ctx = ctx;
...
pdf_obj *
pdf_new_string(base_context *ctx, char *str, int len)
{
	pdf_obj *obj;
	obj = base_malloc(ctx, offsetof(pdf_obj, u.s.buf) + len + 1);
	obj->ctx = ctx;
	obj->refs = 1;
	obj->kind = PDF_STRING;
	obj->u.s.len = len;
...
pdf_obj *
base_new_name(base_context *ctx, char *str)
{
	pdf_obj *obj;
	obj = base_malloc(ctx, offsetof(pdf_obj, u.n) + strlen(str) + 1);
	obj->ctx = ctx;
	obj->refs = 1;
	obj->kind = PDF_NAME;
	strcpy(obj->u.n, str);
...
pdf_obj *
pdf_new_indirect(base_context *ctx, int num, int gen, void *xref)
{
	pdf_obj *obj;
	obj = base_malloc(ctx, sizeof(pdf_obj));
	obj->ctx = ctx;
...
#define RESOLVE(obj) \
	do { \
		...
		{\
			base_assert_lock_not_held(obj->ctx, base_LOCK_FILE); \
...
pdf_obj *
pdf_new_array(base_context *ctx, int initialcap)
{
	pdf_obj *obj;
	int i;
	...
	base_try(ctx)
	{
		obj->u.a.items = base_malloc_array(ctx, obj->u.a.cap, sizeof(pdf_obj*));
	}
	base_catch(ctx)
	{
		base_free(ctx, obj);
...
pdf_array_grow(pdf_obj *obj)
{
	...
	obj->u.a.items = base_resize_array(obj->ctx, obj->u.a.items, new_cap, sizeof(pdf_obj*));
	obj->u.a.cap = new_cap;
...
pdf_obj *
pdf_copy_array(base_context *ctx, pdf_obj *obj)
{
	pdf_obj *arr;
	int i;
	int n;
	...
	if (obj->kind != PDF_ARRAY)
		base_warn(ctx, "assert: not an array (%s)", pdf_objkindstr(obj));
...
pdf_array_put(pdf_obj *obj, int i, pdf_obj *item)
{
	...
	if (!obj)
		return; 
	if (obj->kind != PDF_ARRAY)
		base_warn(obj->ctx, "assert: not an array (%s)", pdf_objkindstr(obj));
	else if (i < 0)
		base_warn(obj->ctx, "assert: index %d < 0", i);
	else if (i >= obj->u.a.len)
		base_warn(obj->ctx, "assert: index %d > length %d", i, obj->u.a.len);
	else
	{
		if (obj->u.a.items[i])
...
pdf_obj *
pdf_new_dict(base_context *ctx, int initialcap)
{
	pdf_obj *obj;
	int i;
	...
	base_try(ctx)
	{
		obj->u.d.items = base_malloc_array(ctx, obj->u.d.cap, sizeof(struct keyval));
	}
	base_catch(ctx)
	{
		base_free(ctx, obj);
...
pdf_dict_grow(pdf_obj *obj)
{
	...
	obj->u.d.items = base_resize_array(obj->ctx, obj->u.d.items, new_cap, sizeof(struct keyval));
	obj->u.d.cap = new_cap;
...
pdf_obj *
pdf_copy_dict(base_context *ctx, pdf_obj *obj)
{
	pdf_obj *dict;
	int i, n;
	...
	if (obj->kind != PDF_DICT)
		base_warn(ctx, "assert: not a dict (%s)", pdf_objkindstr(obj));

	n = pdf_dict_len(obj);
	dict = pdf_new_dict(ctx, n);
	for (i = 0; i < n; i++)
		base_dict_put(dict, pdf_dict_get_key(obj, i), pdf_dict_get_val(obj, i));
...
void
base_dict_put(pdf_obj *obj, pdf_obj *key, pdf_obj *val)
{
	int location;
	char *s;
	int i;
	...
	if (obj->kind != PDF_DICT)
	{
		base_warn(obj->ctx, "assert: not a dict (%s)", pdf_objkindstr(obj));
		return;
	}
	...
	if (!key || key->kind != PDF_NAME)
	{
		base_warn(obj->ctx, "assert: key is not a name (%s)", pdf_objkindstr(obj));
		return;
	}
	else
		s = pdf_to_name(key);
	...
	if (!val)
	{
		base_warn(obj->ctx, "assert: val does not exist for key (%s)", s);
		return;
	}
...
{
	pdf_obj *keyobj = base_new_name(obj->ctx, key);
	base_dict_put(obj, keyobj, val);
	pdf_drop_obj(keyobj);
}
...
pdf_free_array(pdf_obj *obj)
{
	...
	base_free(obj->ctx, obj->u.a.items);
	base_free(obj->ctx, obj);
}
...
pdf_free_dict(pdf_obj *obj)
{
	...
	base_free(obj->ctx, obj->u.d.items);
	base_free(obj->ctx, obj);
}
...
pdf_fprint_obj(FILE *fp, pdf_obj *obj, int tight)
{
	...
	{
		ptr = base_malloc(obj->ctx, n + 1);
		pdf_sprint_obj(ptr, n + 1, obj, tight);
		fputs(ptr, fp);
		fputc('\n', fp);
		base_free(obj->ctx, ptr);
	}
	return n;
}

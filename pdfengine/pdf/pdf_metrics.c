...
void
pdf_set_font_wmode(base_context *ctx, pdf_font_desc *font, int wmode)
{
	font->wmode = wmode;
}

void
pdf_set_default_hmtx(base_context *ctx, pdf_font_desc *font, int w)
{
	font->dhmtx.w = w;
}

void
pdf_set_default_vmtx(base_context *ctx, pdf_font_desc *font, int y, int w)
{
	font->dvmtx.y = y;
	font->dvmtx.w = w;
}
...
void
pdf_add_hmtx(base_context *ctx, pdf_font_desc *font, int lo, int hi, int w)
{
	if (font->hmtx_len + 1 >= font->hmtx_cap)
	{
		int new_cap = font->hmtx_cap + 16;
		font->hmtx = base_resize_array(ctx, font->hmtx, new_cap, sizeof(pdf_hmtx));
...
void
pdf_add_vmtx(base_context *ctx, pdf_font_desc *font, int lo, int hi, int x, int y, int w)
{
	if (font->vmtx_len + 1 >= font->vmtx_cap)
	{
		int new_cap = font->vmtx_cap + 16;
		font->vmtx = base_resize_array(ctx, font->vmtx, new_cap, sizeof(pdf_vmtx));
...
void
pdf_end_hmtx(base_context *ctx, pdf_font_desc *font)
{
	if (!font->hmtx)
		return;
...
void
pdf_end_vmtx(base_context *ctx, pdf_font_desc *font)
{
	if (!font->vmtx)
		return;
...
pdf_hmtx
pdf_lookup_hmtx(base_context *ctx, pdf_font_desc *font, int cid)
{
	int l = 0;
...
pdf_vmtx
pdf_lookup_vmtx(base_context *ctx, pdf_font_desc *font, int cid)
{
	pdf_hmtx h;
	pdf_vmtx v;
	int l = 0;
